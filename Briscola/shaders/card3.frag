...
layout(binding = 1, set = 1) uniform sampler2D uAtlas;
layout(binding = 2, set = 1) uniform sampler2D uBack;
...
void main() {
	...
	vec3 albedo = texture(uAtlas, fragUV).rgb;//* (3.0 + texture(uBack, fragPos.xz)).rgb / 4.0;
